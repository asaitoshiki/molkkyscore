...
| 広告と課金 | バナーとゲーム間の全画面広告、買い切りで広告を消す導線（実配信はネイティブ化後） |
...
- **バナー** — ホーム・履歴・戦績・ルールの下部。スコア入力画面には置かない
  （屋外で誤タップすると試合の記録が壊れるため）
- **全画面** — ゲームの区切りだけ。`INTERSTITIAL_INTERVAL` ゲームおきに 1 回
- **買い切り** — `/remove-ads`。購入後は広告の配信そのものを止める

ストア公開前にやること。

1. Capacitor でアプリ化し、`@capacitor-community/admob` を入れて `AdBanner` と
   `InterstitialAd` の中身を実際の広告に差し替える
